...
if menu_selection == "🌐 Executive Overview":
    ...
    avg_ml_confidence = np.mean([a['ml_probability'] for a in st.session_state.alerts]) if total_alerts > 0 else 0.0
    ...
    with c5:
        st.markdown(f"""
        <div class='kpi-card'>
            <div class='kpi-label'>Avg ML Confidence Score</div>
            <div class='kpi-val' style='-webkit-text-fill-color: #a29bfe;'>{avg_ml_confidence:.2%}</div>
            <div style='color: #a29bfe; font-size: 0.8rem; font-weight:600;'>Model fraud confidence</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col_list:
            ...
            if not filtered_cases_to_show:
                ...
                for a in filtered_cases_to_show:
                    ...
                    btn_label = f"📁 {a['alert_id']} | Risk: {a['risk_score']:.1%} | Model Confidence: {a['ml_probability']:.1%} | {status_badge} | Assignee: {assignee}"
...
# ------------------------------------------------------------------------------
    ...
    tab_sandbox, tab_cluster, tab_hparam = st.tabs(["🔬 Feature Sandbox Simulator", "🔥 Correlation & Clustering", "⚙️ Hyperparameter Tuning"])
    ...
    # --------------------------------------------------------------------------
    # NEW TAB: CORRELATION & CLUSTERING
    # --------------------------------------------------------------------------
    with tab_cluster:
        st.markdown("### Feature Correlation Heatmap & Transaction Clustering")
        st.write("Pearson correlation analysis and K-Means clustering of the top 5 PCA-derived fraud detection features.")

        if raw_data.empty:
            st.error("Data not available for analysis.")
        else:
            col_heat, col_clust = st.columns(2)

            with col_heat:
                st.markdown("#### Pearson Correlation Heatmap")
                st.write("Correlation between V3, V10, V12, V14, V17 across a representative 5,000-transaction sample.")

                sample_corr = raw_data.sample(min(5000, len(raw_data)), random_state=42)
                corr_matrix = sample_corr[feature_names].corr()
            ...
            with col_clust:
                st.markdown("#### K-Means Clustering (V14 vs V12 Space)")
                st.write("3-cluster K-Means grouping on the top 2 importance features. ✕ markers = fraud ground truth.")

                clust_sample = raw_data.sample(min(3000, len(raw_data)), random_state=42)
                X_clust = clust_sample[['v14', 'v12']].values

                kmeans_model = KMeans(n_clusters=3, random_state=42, n_init=10)
                cluster_labels = kmeans_model.fit_predict(X_clust)

                clust_df = pd.DataFrame({
                    'v14': clust_sample['v14'].values,
                    'v12': clust_sample['v12'].values,
                    'Cluster': cluster_labels.astype(str),
                    'Ground Truth': clust_sample['class'].map({0: 'Normal', 1: 'Fraud'}).values
                })
                ...
                fig_clust = px.scatter(
                    ...
                    color='Cluster',
                    symbol='Ground Truth',
                    color_discrete_sequence=['#FF416C', '#2ed573', '#a29bfe'],
                    symbol_map={'Normal': 'circle', 'Fraud': 'x'},
                    opacity=0.65,
                    labels={'v14': 'V14 Feature', 'v12': 'V12 Feature'}
                )
            ...
            clust_summary = clust_df.groupby('Cluster').agg(
                Count=('v14', 'count'),
                Avg_V14=('v14', 'mean'),
                Avg_V12=('v12', 'mean'),
                Fraud_Cases=('Ground Truth', lambda x: (x == 'Fraud').sum())
            ).reset_index()
            clust_summary.columns = ['Cluster ID', 'Transaction Count', 'Avg V14', 'Avg V12', 'Fraud Cases']
            clust_summary['Avg V14'] = clust_summary['Avg V14'].round(4)
            clust_summary['Avg V12'] = clust_summary['Avg V12'].round(4)
            st.dataframe(clust_summary, use_container_width=True)

    # --------------------------------------------------------------------------
    # NEW TAB: HYPERPARAMETER TUNING
    # --------------------------------------------------------------------------
    with tab_hparam:
        st.markdown("### Hyperparameter Sensitivity Analysis")
        st.write("Explore how key Random Forest hyperparameters affect Precision, Recall, and F1-Score on a representative subsample of historical data.")

        if raw_data.empty:
            st.error("Data not available for hyperparameter analysis.")
        else:
            col_hp1, col_hp2 = st.columns([1, 2])

            with col_hp1:
                st.markdown("#### Experiment Configuration")
                hp_param_choice = st.selectbox(
                    "Hyperparameter to Vary",
                    ["n_estimators (Tree Count)", "max_depth (Tree Depth)", "min_samples_split"]
                )
                hp_subsample_size = st.slider("Training Subsample (Normal Txns)", 500, 3000, 1500, 250)
                run_hp_btn = st.button("▶ Run Tuning Experiment", type="primary", use_container_width=True)
                st.info("This trains a series of Random Forest classifiers with varying parameter values and plots performance metrics.")

            with col_hp2:
                st.markdown("#### Performance Results")
                if run_hp_btn:
                    with st.spinner("Running hyperparameter sweep — please wait..."):
                        from sklearn.metrics import precision_score, recall_score, f1_score

                        df_hf = raw_data[raw_data['class'] == 1]
                        df_hn = raw_data[raw_data['class'] == 0].sample(hp_subsample_size, random_state=42)
                        hp_data = pd.concat([df_hf, df_hn]).sample(frac=1, random_state=42)
                        X_hp = hp_data[feature_names].values
                        y_hp = hp_data['class'].values

                        if "n_estimators" in hp_param_choice:
                            param_values = [10, 25, 50, 75, 100, 150, 200]
                            param_key = 'n_estimators'
                        elif "max_depth" in hp_param_choice:
                            param_values = [2, 4, 6, 8, 10, 15, None]
                            param_key = 'max_depth'
                        else:
                            param_values = [2, 5, 10, 15, 20, 30, 50]
                            param_key = 'min_samples_split'

                        hp_results = []
                        split_idx = int(0.7 * len(X_hp))
                        for val in param_values:
                            base_kwargs = {'random_state': 42, 'n_jobs': -1, 'n_estimators': 50}
                            base_kwargs[param_key] = val
                            clf_hp = RandomForestClassifier(**base_kwargs)
                            clf_hp.fit(X_hp[:split_idx], y_hp[:split_idx])
                            y_pred_hp = clf_hp.predict(X_hp[split_idx:])
                            hp_results.append({
                                'Parameter Value': str(val),
                                'Precision': round(precision_score(y_hp[split_idx:], y_pred_hp, zero_division=0), 4),
                                'Recall': round(recall_score(y_hp[split_idx:], y_pred_hp, zero_division=0), 4),
                                'F1 Score': round(f1_score(y_hp[split_idx:], y_pred_hp, zero_division=0), 4)
                            })

                        hp_df = pd.DataFrame(hp_results)

                        fig_hp = go.Figure()
                        for metric, color in [('Precision', '#FF416C'), ('Recall', '#2ed573'), ('F1 Score', '#a29bfe')]:
                            fig_hp.add_trace(go.Scatter(
                                x=hp_df['Parameter Value'],
                                y=hp_df[metric],
                                name=metric,
                                mode='lines+markers',
                        ...
                        fig_hp.update_layout(
                            ...
                            xaxis=dict(title=hp_param_choice, showgrid=True, gridcolor='rgba(255,255,255,0.05)'),
                            yaxis=dict(title='Score', showgrid=True, gridcolor='rgba(255,255,255,0.05)', range=[0, 1.05]),
                            legend=dict(orientation='h', yanchor='bottom', y=1.02, xanchor='right', x=1)
                        )
                    ...
                    st.info("Configure settings on the left and click **▶ Run Tuning Experiment** to start the sweep.")
...
# ------------------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # PERFORMANCE ANALYTICS CHARTS (moved here from Model Governance)
    # ------------------------------------------------------------------
    st.markdown("### 📈 Performance Analytics Charts")
    st.write("Interactive ML performance diagnostics for compliance auditing and reporting, evaluated on a stratified subsample of 20,000+ historical transactions.")

    if not raw_data.empty and rf_model:
        with st.spinner("Generating performance analytics charts..."):
            df_rfraud = raw_data[raw_data['class'] == 1]
            df_rnormal = raw_data[raw_data['class'] == 0].sample(20000, random_state=42)
            rpt_val_df = pd.concat([df_rfraud, df_rnormal])
            X_rpt = rpt_val_df[feature_names]
            X_rpt_ordered = X_rpt[rf_model.feature_names_in_]
            y_rpt = rpt_val_df['class']
            y_rpt_probs = rf_model.predict_proba(X_rpt_ordered)[:, 1]

            fpr_rpt, tpr_rpt, _ = roc_curve(y_rpt, y_rpt_probs)
            roc_auc_rpt = auc(fpr_rpt, tpr_rpt)
            prec_rpt, rec_rpt, _ = precision_recall_curve(y_rpt, y_rpt_probs)
            pr_auc_rpt = auc(rec_rpt, prec_rpt)

            col_roc_r, col_pr_r = st.columns(2)
            with col_roc_r:
                st.markdown("#### ROC Curve")
                fig_roc_rpt = go.Figure()
                fig_roc_rpt.add_trace(go.Scatter(
                    x=fpr_rpt, y=tpr_rpt, mode='lines',
                    name=f'RF Model (AUC={roc_auc_rpt:.4f})',
                    line=dict(color='#ff4757', width=2)
                ))
                fig_roc_rpt.add_trace(go.Scatter(
                    x=[0, 1], y=[0, 1], mode='lines',
                    name='Baseline', line=dict(color='#8892b0', dash='dash')
                ))
                fig_roc_rpt.update_layout(
                ...
                st.plotly_chart(fig_roc_rpt, use_container_width=True)

            with col_pr_r:
                st.markdown("#### Precision-Recall Curve")
                fig_pr_rpt = go.Figure()
                fig_pr_rpt.add_trace(go.Scatter(
                    x=rec_rpt, y=prec_rpt, mode='lines',
                    name=f'RF Model (AUC={pr_auc_rpt:.4f})',
                    line=dict(color='#2ed573', width=2)
                ))
                fig_pr_rpt.update_layout(
                    paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
                    margin=dict(l=0, r=0, t=10, b=0),
                    xaxis=dict(title='Recall Rate', showgrid=True, gridcolor='rgba(255,255,255,0.05)'),
                    yaxis=dict(title='Precision', showgrid=True, gridcolor='rgba(255,255,255,0.05)')
                )
                st.plotly_chart(fig_pr_rpt, use_container_width=True)

            col_cm_r, col_fi_r = st.columns(2)
            with col_cm_r:
                st.markdown("#### Confusion Matrix")
                y_rpt_preds = (y_rpt_probs >= 0.5).astype(int)
                cm_rpt = confusion_matrix(y_rpt, y_rpt_preds)
                fig_cm_rpt = px.imshow(
                    cm_rpt,
                ...
                fig_cm_rpt.update_layout(
                    paper_bgcolor='rgba(0,0,0,0)',
                    margin=dict(l=0, r=0, t=10, b=0)
                )
                st.plotly_chart(fig_cm_rpt, use_container_width=True)

            with col_fi_r:
                st.markdown("#### Global Feature Importance")
                fi_rpt = rf_model.feature_importances_
                fi_df_rpt = pd.DataFrame({
                    'Feature': rf_model.feature_names_in_,
                    'Gini Importance': fi_rpt
                }).sort_values('Gini Importance', ascending=True)
                fig_fi_rpt = px.bar(
                    fi_df_rpt, x='Gini Importance', y='Feature',
                    orientation='h', color='Gini Importance',
                    color_continuous_scale='Turbo'
                )
                fig_fi_rpt.update_layout(
                    paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
                    margin=dict(l=0, r=0, t=10, b=0),
                    xaxis=dict(showgrid=True, gridcolor='rgba(255,255,255,0.05)')
                )
                st.plotly_chart(fig_fi_rpt, use_container_width=True)
    else:
        st.info("Model or data not loaded. Charts will appear once transactions are populated.")
